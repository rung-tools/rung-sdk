...
export const Char = (length: number) => ({ name: 'Char', length });
export const IntegerRange = (from: number, to: number) => ({ name: 'IntegerRange', from, to });
export const DoubleRange = (from: number, to: number) => ({ name: 'DoubleRange', from, to });
...
export const OneOf = (values: string[]) => ({ name: 'OneOf', values });
